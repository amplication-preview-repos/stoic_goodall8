...
export class ClassModelServiceBase {
  ...
  async getObjects(parentId: string): Promise<PrismaObject | null> {
  ...
  async getProperties(parentId: string): Promise<PrismaProperty | null> {
    return this.prisma.classModel
      .findUnique({
        where: { id: parentId },
      })
  ...
  async getOntologies(parentId: string): Promise<PrismaOntology | null> {
    return this.prisma.classModel
      .findUnique({
        where: { id: parentId },
      })
